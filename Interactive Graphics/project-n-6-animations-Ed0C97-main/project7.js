...
function SimTimeStep(dt, positions, velocities, springs, springStiffness, damping, particleMass, gravity, restitution) {
	...
	const forces = new Array(numParticles).fill(null).map(() => new Vec3(0, 0, 0));
	...
	for (let spring of springs) {
		...
		const delta = pos1.sub(pos0);
		const dist = delta.len();
		const direction = delta.unit();
		const springForce = direction.mul(springStiffness * (dist - restLength));
	...
	for (let i = 0; i < numParticles; i++) {
		...
		forces[i].dec(velocities[i].mul(damping));
	}
